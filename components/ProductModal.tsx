...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div 
        className="bg-white w-full sm:max-w-2xl rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh] border border-slate-200 animate-in fade-in duration-150"
          ...
          <div className="space-y-2 pt-1">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="tap-press text-xs font-semibold text-slate-900 hover:underline flex items-center gap-1"
              >
            ...
            <div className="space-y-2.5">
              {variants.map((variant, varIdx) => {
                return (
                  <div key={varIdx} className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 flex-1">
                        ...
                        <input
                          ...
                          placeholder="e.g. T.Blue / Sh No.02"
                          className="w-full px-2.5 py-1 bg-white border border-slate-200 rounded text-xs font-semibold text-slate-900 focus:outline-none focus:border-slate-900"
                        />
                      </div>
                      {variants.length > 1 && (
                        <button
                    ...
                    <div className="grid grid-cols-5 gap-1.5 pt-0.5">
                      {STANDARD_SIZES.map((sz) => (
                        <div key={sz} className="bg-white rounded-lg p-1.5 border border-slate-200 text-center">
                          ...
                          <input
                            type="number"
                            min="0"
                            value={variant.sizes[sz] ?? 0}
                            onChange={(e) => handleVariantSizeChange(varIdx, sz, e.target.value)}
                            className="w-full text-center font-mono font-bold num-tabular text-slate-900 text-xs py-0.5 bg-slate-50 border border-slate-100 rounded focus:outline-none focus:border-slate-900"
                          />
                        </div>
                      ))}
                    </div>
